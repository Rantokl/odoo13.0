...
class ViseoProjectTaskType(models.Model):
    _name = 'viseo.project.task.type'
    _description = 'Task Stage'
    _order = 'sequence, id'

    def _get_default_project_ids(self):
        default_project_id = self.env.context.get('default_viseo_project_id')
        return [default_project_id] if default_project_id else None

    name = fields.Char(string='Stage Name', required=True, translate=True)
    description = fields.Text(translate=True)
    sequence = fields.Integer(default=1)
    project_ids = fields.Many2many('viseo.project.project', 'viseo_project_task_type_rel', 'viseo_type_id', 'viseo_project_id', string='Projets',
    ...
    mail_template_id = fields.Many2one(
        ...
        domain=[('model', '=', 'viseo.project.task')],
    ...
    def unlink(self):
        ...
        if default_project_id:
            ...
            tasks = self.env['viseo.project.task'].with_context(active_test=False).search([('viseo_project_id', '=', default_project_id), ('stage_id', 'in', self.ids)])
        ...
        return super(ViseoProjectTaskType, stages).unlink()


class ViseoProject(models.Model):
    _name = "viseo.project.project"
    ...
    def get_alias_model_name(self, vals):
        return vals.get('alias_model', 'viseo.project.task')

    def get_alias_values(self):
        values = super(ViseoProject, self).get_alias_values()
        values['alias_defaults'] = {'viseo_project_id': self.id}
        return values
    ...
    def _compute_attached_docs_count(self):
        ...
        for project in self:
            project.doc_count = Attachment.search_count([
                '|',
                '&',
                ('res_model', '=', 'viseo.project.project'), ('res_id', '=', project.id),
                '&',
                ('res_model', '=', 'viseo.project.task'), ('res_id', 'in', project.task_ids.ids)
            ])

    def _compute_task_count(self):
        task_data = self.env['viseo.project.task'].read_group([('viseo_project_id', 'in', self.ids), '|', ('stage_id.fold', '=', False), ('stage_id', '=', False)], ['viseo_project_id'], ['viseo_project_id'])
        result = dict((data['viseo_project_id'][0], data['viseo_project_id_count']) for data in task_data)
        for project in self:
    ...
    def activate_sample_project(self):
        """ Unarchives the sample project 'viseo.project.project_project_data' and
        ...
        cover_image = self.env.ref('viseo_project_project.msg_task_data_14_attach', False)
        ...
        action = self.env.ref('viseo_project_project.open_view_project_all2', False)
        action_data = None
        if action:
            action.sudo().write({
    ...
    def _inverse_is_favorite(self):
        favorite_projects = not_fav_projects = self.env['viseo.project.project'].sudo()
        for project in self:
    ...
    favorite_user_ids = fields.Many2many(
        'res.users', 'viseo_project_favorite_user_rel', 'viseo_project_id', 'viseo_user_id',
    ...
    tasks = fields.One2many('viseo.project.task', 'viseo_project_id', string="Task Activities")
    ...
    type_ids = fields.Many2many('viseo.project.task.type', 'viseo_project_task_type_rel', 'viseo_project_id', 'viseo_type_id', string='Tasks Stages')
    task_count = fields.Integer(compute='_compute_task_count', string="Task Count")
    task_ids = fields.One2many('viseo.project.task', 'viseo_project_id', string='Tasks',
    ...
    subtask_viseo_project_id = fields.Many2one('viseo.project.project', string='Sub-task Project', ondelete="restrict",
    ...
    def _compute_access_url(self):
        super(ViseoProject, self)._compute_access_url()
        for project in self:
            project.access_url = '/my/project/%s' % project.id

    def _compute_access_warning(self):
        super(ViseoProject, self)._compute_access_warning()
    ...
    def map_tasks(self, new_project_id):
        ...
        tasks = self.env['viseo.project.task']
        # We want to copy archived task, but do not propagate an active_test context key
        task_ids = self.env['viseo.project.task'].with_context(active_test=False).search([('viseo_project_id', '=', self.id)], order='parent_id').ids
        old_to_new_tasks = {}
        for task in self.env['viseo.project.task'].browse(task_ids):
    ...
    def copy(self, default=None):
        ...
        project = super(ViseoProject, self).copy(default)
        if self.subtask_viseo_project_id == self:
            project.subtask_viseo_project_id = project
    ...
    def create(self, vals):
        ...
        project = super(ViseoProject, self).create(vals)
        if not vals.get('subtask_viseo_project_id'):
            project.subtask_viseo_project_id = project.id
    ...
    def write(self, vals):
        ...
        res = super(ViseoProject, self).write(vals) if vals else True
        if 'active' in vals:
    ...
    def unlink(self):
        ...
        result = super(ViseoProject, self).unlink()
        analytic_accounts_to_delete.unlink()
        return result

    def message_subscribe(self, partner_ids=None, channel_ids=None, subtype_ids=None):
        """ Subscribe to all existing active tasks when subscribing to a project """
        res = super(ViseoProject, self).message_subscribe(partner_ids=partner_ids, channel_ids=channel_ids, subtype_ids=subtype_ids)
    ...
    def message_unsubscribe(self, partner_ids=None, channel_ids=None):
        ...
        return super(ViseoProject, self).message_unsubscribe(partner_ids=partner_ids, channel_ids=channel_ids)
    ...
    def open_tasks(self):
        ...
        ctx.update({'search_default_project_id': self.id, 'default_viseo_project_id': self.id})
        action = self.env['ir.actions.act_window'].for_xml_id('viseo_project_project', 'act_project_project_2_project_task_all')
    ...
    def action_view_all_rating(self):
        ...
        action = self.env['ir.actions.act_window'].for_xml_id('viseo_project_project', 'rating_rating_action_view_project_rating')
...
class ViseoTask(models.Model):
    ...
    def search_panel_select_range(self, field_name, project_id=None):
        all_projects = super(ViseoTask, self).search_panel_select_range(field_name)
        if project_id and self._name == 'viseo.project.task' and field_name == 'viseo_project_id':
    ...
    def default_get(self, fields_list):
        result = super(ViseoTask, self).default_get(fields_list)
    ...
    def _get_default_partner(self):
        if 'default_viseo_project_id' in self.env.context:
            default_project_id = self.env['viseo.project.project'].browse(self.env.context['default_viseo_project_id'])
            return default_project_id.exists().partner_id

    def _get_default_stage_id(self):
        """ Gives default stage_id """
        project_id = self.env.context.get('default_viseo_project_id')
        if not project_id:
            return False
    ...
    def _default_company_id(self):
        if self._context.get('default_viseo_project_id'):
            return self.env['viseo.project.project'].browse(self._context['default_viseo_project_id']).company_id
    ...
    def _read_group_stage_ids(self, stages, domain, order):
        ...
        if 'default_viseo_project_id' in self.env.context:
            search_domain = ['|', ('project_ids', '=', self.env.context['default_viseo_project_id'])] + search_domain
    ...
    stage_id = fields.Many2one('viseo.project.task.type', string='Stage', ondelete='restrict', tracking=True, index=True,
        default=_get_default_stage_id, group_expand='_read_group_stage_ids',
        domain="[('project_ids', '=', viseo_project_id)]", copy=False)
    # tag_ids = fields.Many2many('viseo.project.tags', string='Etiquettes')
    ...
    date_last_stage_update = fields.Datetime(string='Last Stage Update',
        ...
        copy=False,
        readonly=True)
    viseo_project_id = fields.Many2one('viseo.project.project', string='Project', default=lambda self: self.env.context.get('default_viseo_project_id'),
    ...
    manager_id = fields.Many2one('res.users', string='Project Manager', related='viseo_project_id.user_id', readonly=True, related_sudo=False)
    ...
    displayed_image_id = fields.Many2one('ir.attachment', domain="[('res_model', '=', 'viseo.project.task'), ('res_id', '=', id), ('mimetype', 'ilike', 'image')]", string='Cover Image')
    legend_blocked = fields.Char(related='stage_id.legend_blocked', string='Kanban Blocked Explanation', readonly=True, related_sudo=False)
    legend_done = fields.Char(related='stage_id.legend_done', string='Kanban Valid Explanation', readonly=True, related_sudo=False)
    legend_normal = fields.Char(related='stage_id.legend_normal', string='Kanban Ongoing Explanation', readonly=True, related_sudo=False)
    parent_id = fields.Many2one('viseo.project.task', string='Parent Task', index=True)
    child_ids = fields.One2many('viseo.project.task', 'parent_id', string="Sub-tasks", context={'active_test': False})
    subtask_viseo_project_id = fields.Many2one('viseo.project.project', related="viseo_project_id.subtask_viseo_project_id", string='Sub-task Project', readonly=True)
    ...
    def _compute_attachment_ids(self):
        for task in self:
            attachment_ids = self.env['ir.attachment'].search([('res_id', '=', task.id), ('res_model', '=', 'viseo.project.task')]).ids
    ...
    def _compute_elapsed(self):
        task_linked_to_calendar = self.filtered(
            lambda task: task.viseo_project_id.resource_calendar_id and task.create_date
        )
        for task in task_linked_to_calendar:
            ...
            if task.date_assign:
                ...
                duration_data = task.viseo_project_id.resource_calendar_id.get_work_duration_data(dt_create_date, dt_date_assign, compute_leaves=True)
            ...
            if task.date_end:
                ...
                duration_data = task.viseo_project_id.resource_calendar_id.get_work_duration_data(dt_create_date, dt_date_end, compute_leaves=True)
    ...
    def _compute_access_url(self):
        super(ViseoTask, self)._compute_access_url()
        for task in self:
            task.access_url = '/my/task/%s' % task.id

    def _compute_access_warning(self):
        super(ViseoTask, self)._compute_access_warning()
        for task in self.filtered(lambda x: x.viseo_project_id.privacy_visibility != 'portal'):
    ...
    def _compute_subtask_count(self):
        ...
        task_data = self.env['viseo.project.task'].read_group([('parent_id', 'in', self.ids)], ['parent_id'], ['parent_id'])
    ...
    @api.onchange('viseo_project_id')
    def _onchange_project(self):
        if self.viseo_project_id:
            # find partner
            if self.viseo_project_id.partner_id:
                self.partner_id = self.viseo_project_id.partner_id
            # find stage
            if self.viseo_project_id not in self.stage_id.project_ids:
                self.stage_id = self.stage_find(self.viseo_project_id.id, [('fold', '=', False)])
            # keep multi company consistency
            self.company_id = self.viseo_project_id.company_id
        else:
            self.stage_id = False
    
    @api.onchange('company_id')
    def _onchange_task_company(self):
        if self.viseo_project_id.company_id != self.company_id:
            self.viseo_project_id = False
    ...
    def copy(self, default=None):
        ...
        if not default.get('name'):
            default['name'] = _("%s (copy)") % self.name
        return super(ViseoTask, self).copy(default)
    ...
    def get_empty_list_help(self, help):
        tname = _("task")
        project_id = self.env.context.get('default_viseo_project_id', False)
        if project_id:
            name = self.env['viseo.project.project'].browse(project_id).label_tasks
            if name: tname = name.lower()

        self = self.with_context(
            empty_list_help_id=self.env.context.get('default_viseo_project_id'),
            empty_list_help_model='viseo.project.project',
            empty_list_help_document_name=tname,
        )
        return super(ViseoTask, self).get_empty_list_help(help)
    ...
    def stage_find(self, section_id, domain=[], order='sequence'):
        ...
        section_ids.extend(self.mapped('viseo_project_id').ids)
        search_domain = []
        if section_ids:
        ...
        return self.env['viseo.project.task.type'].search(search_domain, order=order, limit=1).id
    ...
    def write(self, vals):
        ...
        result = super(ViseoTask, self).write(vals)
        # rating on stage
        if 'stage_id' in vals and vals.get('stage_id'):
            self.filtered(lambda x: x.viseo_project_id.rating_status == 'stage')._send_task_rating_mail(force_send=True)
        return result

    def update_date_end(self, stage_id):
        project_task_type = self.env['viseo.project.task.type'].browse(stage_id)
    ...
    def _subtask_values_from_parent(self, parent_id):
        ...
        result = {}
        parent_task = self.env['viseo.project.task'].browse(parent_id)
        ...
        result['viseo_project_id'] = parent_task.viseo_project_id.subtask_viseo_project_id
    ...
    def _track_template(self, changes):
        res = super(ViseoTask, self)._track_template(changes)
        test_task = self[0]
    ...
    def _track_subtype(self, init_values):
        ...
        if 'kanban_state_label' in init_values and self.kanban_state == 'blocked':
            return self.env.ref('viseo_project_project.mt_task_blocked')
        elif 'kanban_state_label' in init_values and self.kanban_state == 'done':
            return self.env.ref('viseo_project_project.mt_task_ready')
        elif 'stage_id' in init_values:
            return self.env.ref('viseo_project_project.mt_task_stage')
        return super(ViseoTask, self)._track_subtype(init_values)
    ...
    def _notify_get_groups(self, msg_vals=None):
        ...
        groups = super(ViseoTask, self)._notify_get_groups(msg_vals=msg_vals)
        local_msg_vals = dict(msg_vals or {})
        self.ensure_one()

        project_user_group_id = self.env.ref('viseo_project_project.group_project_user_viseo').id
        new_group = (
            'group_project_user',
    ...
    def _notify_get_reply_to(self, default=None, records=None, company=None, doc_names=None):
        ...
        aliases = self.sudo().mapped('viseo_project_id')._notify_get_reply_to(default=default, records=None, company=company, doc_names=None)
        res = {task.id: aliases.get(task.viseo_project_id.id) for task in self}
        leftover = self.filtered(lambda rec: not rec.viseo_project_id)
        if leftover:
            res.update(super(ViseoTask, leftover)._notify_get_reply_to(default=default, records=None, company=company, doc_names=doc_names))
        return res

    def email_split(self, msg):
        email_list = tools.email_split((msg.get('to') or '') + ',' + (msg.get('cc') or ''))
        # check left-part is not already an alias
        aliases = self.mapped('viseo_project_id.alias_name')
    ...
    def message_new(self, msg, custom_values=None):
        ...
        task = super(ViseoTask, self.with_context(create_context)).message_new(msg, custom_values=defaults)
    ...
    def message_update(self, msg, update_vals=None):
        ...
        return super(ViseoTask, self).message_update(msg, update_vals=update_vals)

    def _message_get_suggested_recipients(self):
        recipients = super(ViseoTask, self)._message_get_suggested_recipients()
        for task in self:
            if task.partner_id:
    ...
    def _notify_email_header_dict(self):
        headers = super(ViseoTask, self)._notify_email_header_dict()
        if self.viseo_project_id:
            current_objects = [h for h in headers.get('X-Odoo-Objects', '').split(',') if h]
            current_objects.insert(0, 'viseo.project.project-%s, ' % self.viseo_project_id.id)
    ...
    def _message_post_after_hook(self, message, msg_vals):
        ...
        return super(ViseoTask, self)._message_post_after_hook(message, msg_vals)
    ...
    def action_subtask(self):
        action = self.env.ref('viseo_project_project.project_task_action_sub_task2').read()[0]
        ...
        if self._context.get('default_viseo_project_id'):
            default_project = self.env['viseo.project.project'].browse(self.env.context['default_viseo_project_id'])
        else:
            default_project = self.viseo_project_id.subtask_viseo_project_id or self.viseo_project_id
    ...
    def rating_get_partner_id(self):
        res = super(ViseoTask, self).rating_get_partner_id()
        if not res and self.viseo_project_id.partner_id:
            return self.viseo_project_id.partner_id
        return res

    def rating_apply(self, rate, token=None, feedback=None, subtype=None):
        return super(ViseoTask, self).rating_apply(rate, token=token, feedback=feedback, subtype="project.mt_task_rating")

    def _rating_get_parent_field_name(self):
        return 'viseo_project_id'
    ...
    project_color = fields.Integer('Project color', related='viseo_project_id.color')
